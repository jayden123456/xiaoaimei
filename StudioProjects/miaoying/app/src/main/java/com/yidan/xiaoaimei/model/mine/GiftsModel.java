...
public class GiftsModel implements GiftsContract.IGiftsModel {

    @Override
    public void getGifts(String uid, final OnHttpCallBack<GiftListInfo> callBack) {
        //网络请求
        RetrofitUtils.newInstence(Const.baseUrl)
                ...
                .subscribe(new Subscriber<GiftListInfo>() {
                    ...
                    @Override
                    public void onNext(GiftListInfo giftInfo) {
                        if (giftInfo.getStatus_code() == 0) {
                            callBack.onSuccessful(giftInfo);
                        } else {
                            callBack.onFaild(giftInfo.getMessage());
                        }
                    }
                });
    }
