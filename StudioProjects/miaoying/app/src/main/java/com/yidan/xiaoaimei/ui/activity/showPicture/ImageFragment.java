package com.yidan.xiaoaimei.ui.activity.showPicture;
...
public class ImageFragment extends BaseFragment {
    ...
    public void initData() {
        ...
        bitMapHight = hight;
        imgPhotoview.setDrawingCacheEnabled(true);
...
//		Logger.i(Tag, Tag+" showPhoto position:" + position);
         ...
         * 第一张图未加载出来背景透明
         */
        if (position == ((ShowPictureActivity) mActivity).getPositon() && ((ShowPictureActivity) mActivity).isFirst()) {
            headBigImage.setBackgroundColor(Color.TRANSPARENT);
        } else {
            headBigImage.setBackgroundColor(Color.BLACK);
        }

        /**
         * 设置photo的初始大小
         */
        imgPhotoview.getLayoutParams().width = width;
        imgPhotoview.getLayoutParams().height = hight;


        RelativeLayout.LayoutParams progressBarParams = (RelativeLayout.LayoutParams) progressBar.getLayoutParams();
        progressBarParams.width = NormalUtil.dip2px(mActivity, 40);
        progressBarParams.height = NormalUtil.dip2px(mActivity, 40);
         ...
         * 判断加载大图还是缩略图
         */
        boolean originalBitmapExist = BitmapUtil.checkImageExist(ImageStringUtil.getOrg(imgData));
        if (originalBitmapExist) {
            RelativeLayout.LayoutParams photoViewParams = (RelativeLayout.LayoutParams) imgPhotoview
                    .getLayoutParams();
            photoViewParams.topMargin = (int) y;
            photoViewParams.leftMargin = (int) x;
            imgPhotoview.setLayoutParams(photoViewParams);
            loadOrgImg(ImageStringUtil.getOrg(imgData), false);

        } else {
            loadThumbImg(ImageStringUtil.getThumb(imgData, width, hight));
        }
    ...
    public void loadThumbImg(final String url) {
        ImageLoader.getInstance().displayImage(url, imgPhotoview, new ImageLoadingListener() {
            @Override
            ...
            public void onLoadingFailed(String s, View view, FailReason failReason) {
                imgPhotoview.setImageResource(R.color.colorEF);
                centerView();
            }
...
//						Logger.i(Tag, Tag+" v_parent hight:"+v_parent.getHeight() );
                        if (is_center) {
                            //photoView.setBackgroundColor(mActivity.getResources().getColor(R.color.black));
...
//						Logger.i(Tag, Tag+" onLoadingComplete bitmap:"+bitmap.getWidth()+":"+bitmap.getHeight());
                        ...
                        if (is_center) {//缩略图在中心位置，原图加载出来后要替换缩略图，尺寸比例大多会发生变化（正方形变成矩形），这里要重新设置photo的一些值，使原图也是居中显示
                            ...
                            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) imgPhotoview
                                    .getLayoutParams();
...
//							Logger.i(Tag, Tag+" center");
                            ...
                            imgPhotoview.getLocationInWindow(location);
                            int x1 = location[0];
                            int y1 = location[1];
...
//						Logger.i(Tag, Tag+" onLoadingCancelled position:"+position);
    ...
    public void setBgAlphaAnimation() {
        if (position == ((ShowPictureActivity) mActivity).getPositon() && ((ShowPictureActivity) mActivity).isFirst()) {
            ((ShowPictureActivity) mActivity).setFirst(false);
            headBigImage.setBackgroundColor(Color.BLACK);
            ...
            headBigImage.startAnimation(bgAlphaAnimation);
    ...
    public void centerView() {
        ...
        //第一张看到的图且第一次加载的动画
        if (((ShowPictureActivity) mActivity).isFirst() && ((ShowPictureActivity) mActivity).getPositon() == position) {
            setBgAlphaAnimation();
            ...
            valueAnimator.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) imgPhotoview.getLayoutParams();
                    ...
                    imgPhotoview.setLayoutParams(params);
            ...
            valueAnimator.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animator) {
                     ...
                     * 居中动画结束，加载原图
                     */
                    loadOrgImg(ImageStringUtil.getOrg(imgData), true);
                }
...
//		Logger.i(Tag, Tag+"fullScreenrx:"+x+"y:"+y);
        ...
        if (((ShowPictureActivity) mActivity).isFirst() && ((ShowPictureActivity) mActivity).getPositon() == position
                || (from_center && ((ShowPictureActivity) mActivity).getCurrentPosition() == position)) {//第一张要显示的图或者居中的且是当前看到的位置
            setBgAlphaAnimation();
            valueAnimator = ValueAnimator.ofFloat(0, 100);
            valueAnimator.addUpdateListener(new AnimatorUpdateListener() {
                //持有一个IntEvaluator对象，方便下面估值的时候使用
    ...
    public void close() {
         ...
         */
        headBigImage.setBackgroundColor(Color.TRANSPARENT);
        imgPhotoview.setBackgroundColor(Color.TRANSPARENT);
        mAttacher.setScaleType(ScaleType.CENTER_CROP);
        /*if (closeScaleType == 1) {
            mAttacher.setScaleType(ScaleType.CENTER_INSIDE);
		}else {
        ...
        valueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            @Override
            public void onAnimationEnd(Animator animator) {
                ((ShowPictureActivity) mActivity).finishAct();
            }
...
//						Logger.i(Tag,Tag + " onLongClick");
    ...
    public void finishFlash() {
        ((ShowPictureActivity) mActivity).setFinish(true);
        if (((ShowPictureActivity) mActivity).isAnimation() || position == ((ShowPictureActivity) mActivity).getPositon()) {
            close();
        } else {
            ((ShowPictureActivity) mActivity).finishAct();
        }
