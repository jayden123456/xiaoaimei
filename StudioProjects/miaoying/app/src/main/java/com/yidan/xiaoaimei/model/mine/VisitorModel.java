...
public class VisitorModel implements VisitorContract.IVisitorModel {

    @Override
    public void getVisitors(String token, String pageNum, final OnHttpCallBack<VisitorInfo> callBack) {
        //网络请求
        RetrofitUtils.newInstence(Const.baseUrl)
                .create(APIService.class)
                .getVisitors(token, pageNum)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<VisitorInfo>() {
                    @Override
                    ...
                    public void onNext(VisitorInfo visitorInfo) {
                        if (visitorInfo.getStatus_code() == 0) {
                            callBack.onSuccessful(visitorInfo);
                        } else {
                            callBack.onFaild(visitorInfo.getMessage());
                        }
                    }
                });
    }
