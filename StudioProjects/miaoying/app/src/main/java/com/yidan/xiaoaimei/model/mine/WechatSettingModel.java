...
public class WechatSettingModel implements WechatSettingContract.IWechatSettingModel {

    @Override
    public void getWechatPay(String token, final OnHttpCallBack<WechatPayInfo> callBack) {
        //网络请求
        RetrofitUtils.newInstence(Const.baseUrl)
                ...
                .subscribe(new Subscriber<WechatPayInfo>() {
                    @Override
                    ...
                    public void onNext(WechatPayInfo wechatPayInfo) {
                        if (wechatPayInfo.getStatus_code() == 0) {
                            callBack.onSuccessful(wechatPayInfo);
                        } else {
                            callBack.onFaild(wechatPayInfo.getMessage());
                        }
                    }
                });
    }
    ...
    @Override
    public void setWechat(String token, String optionId, String wechatNum, final OnHttpCallBack<CommonEmptyInfo> callBack) {
        //网络请求
        RetrofitUtils.newInstence(Const.baseUrl)
                .create(APIService.class)
                .settingWechat(token, optionId, wechatNum)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
